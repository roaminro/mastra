...
Currently, to ensure proper compatibility between Mastra and Assistant UI, you need to setup a server middleware. Update your `src/mastra/index.ts` file with the following configuration:

```typescript showLineNumbers copy filename="src/mastra/index.ts"
export const mastra = new Mastra({
  // mastra server middleware configuration
  server: {
    middleware: [
      {
...
This middleware ensures that when Assistant UI sends a request with `tools: {}` in the request body, we remove those properties to make the request work properly with Mastra.

<Callout type="info">
Passing `tools: {}` from Assiant UI overrides Mastra's built-in tools. Mastra only supports `clientTools` via the Mastra client SDK from the client side. For more information about client tools, see the [Client Tools documentation](/reference/client-js/agents#client-tools).
</Callout>
